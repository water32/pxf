...
public class HdfsUtilities {
    ...
    public static FileSplit parseFileSplit(RequestContext context) {
        ...
        if (context.getFragmentMetadata() == null) {
            metadata = new FragmentMetadata(0, 0);
        } else {
            ByteBuffer buffer = ByteBuffer.allocate(2 * Long.BYTES);
            buffer.put(context.getFragmentMetadata());
            buffer.flip(); // need flip
            metadata = new FragmentMetadata(buffer.getLong(), buffer.getLong());
        }
        return new FileSplit(new Path(context.getDataSource()),
