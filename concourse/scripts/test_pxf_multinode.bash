...
EOF
	...
	ssh "${SSH_OPTS[@]}" gpadmin@cdw "
		...
		if [[ ! -d ${BASE_DIR} ]]; then
			...
			cp ${SERVER_TEMPLATES_DIR}/{hdfs,mapred,yarn,core,hbase,hive}-site.xml ${BASE_DIR}/servers/default
			sed -i -e 's/\(0.0.0.0\|localhost\|127.0.0.1\)/${hadoop_ip}/g' ${BASE_DIR}/servers/default/*-site.xml
		else
			cp ${SERVER_TEMPLATES_DIR}/mapred{,1}-site.xml
		fi &&
		mkdir -p ${BASE_DIR}/servers/s3{,-invalid} &&
		cp ${SERVER_TEMPLATES_DIR}/s3-site.xml ${BASE_DIR}/servers/s3 &&
		cp ${SERVER_TEMPLATES_DIR}/s3-site.xml ${BASE_DIR}/servers/s3-invalid &&
		...
		cp ${SERVER_TEMPLATES_DIR}/jdbc-site.xml ${BASE_DIR}/servers/database/ &&
		...
		cp ${SERVER_TEMPLATES_DIR}/jdbc-site.xml ${BASE_DIR}/servers/db-session-params &&
		sed -i  -e 's|YOUR_DATABASE_JDBC_DRIVER_CLASS_NAME|org.postgresql.Driver|' \
			-e 's|YOUR_DATABASE_JDBC_URL|jdbc:postgresql://cdw:5432/pxfautomation|' \
		...
		cp ${SERVER_TEMPLATES_DIR}/jdbc-site.xml ${BASE_DIR}/servers/db-hive &&
		...
		if [[ \"${PROTOCOL}\" == \"file\" ]]; then
			...
			cp ${SERVER_TEMPLATES_DIR}/mapred-site.xml ${BASE_DIR}/servers/file
			cp ${SERVER_TEMPLATES_DIR}/pxf-site.xml ${BASE_DIR}/servers/file
			sed -i \
...
function setup_pxf_kerberos_on_cluster() {
	...
	sed -i  -e "s|</hdfs>|<hadoopRoot>$DATAPROC_DIR</hadoopRoot></hdfs>|g" \
		...
		"$multiNodesCluster"
	ssh gpadmin@cdw "
		cp ${SERVER_TEMPLATES_DIR}/pxf-site.xml ${BASE_DIR}/servers/db-hive/pxf-site.xml &&
	...
	if [[ -d dataproc_2_env_files ]]; then
		...
		ssh gpadmin@cdw "
			...
			cp ${SERVER_TEMPLATES_DIR}/pxf-site.xml ${BASE_DIR}/servers/hdfs-secure &&
		...
		ssh "${SSH_OPTS[@]}" gpadmin@cdw "
			...
			cp ${SERVER_TEMPLATES_DIR}/jdbc-site.xml ${BASE_DIR}/servers/db-hive-kerberos &&
			sed -i -e 's|YOUR_DATABASE_JDBC_DRIVER_CLASS_NAME|org.apache.hive.jdbc.HiveDriver|' \
			...
			cp ${SERVER_TEMPLATES_DIR}/pxf-site.xml ${BASE_DIR}/servers/db-hive-kerberos/pxf-site.xml &&
	...
	if [[ -d ipa_env_files ]]; then
		...
		ssh gpadmin@cdw "
			...
			cp ${SERVER_TEMPLATES_DIR}/pxf-site.xml ${BASE_DIR}/servers/hdfs-ipa &&
			sed -i \
	...
	ssh gpadmin@cdw "
		...
		cp ${SERVER_TEMPLATES_DIR}/jdbc-site.xml ${BASE_DIR}/servers/db-hive-non-secure &&
		sed -i -e 's|YOUR_DATABASE_JDBC_DRIVER_CLASS_NAME|org.apache.hive.jdbc.HiveDriver|' \
		...
		cp ${SERVER_TEMPLATES_DIR}/{hdfs,mapred,yarn,core,hbase,hive,pxf}-site.xml ${BASE_DIR}/servers/hdfs-non-secure &&
	...
	ssh gpadmin@cdw "
		...
		cp ${SERVER_TEMPLATES_DIR}/pxf-site.xml ${BASE_DIR}/servers/secure-hdfs-invalid-principal &&
	...
	ssh gpadmin@cdw "
		...
		cp ${SERVER_TEMPLATES_DIR}/pxf-site.xml ${BASE_DIR}/servers/secure-hdfs-invalid-keytab &&
