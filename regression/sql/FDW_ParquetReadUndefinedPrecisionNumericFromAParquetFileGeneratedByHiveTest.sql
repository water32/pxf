...
\!{{ HCFS_CMD }} --config {{ PXF_CONF }}/servers/{{ SERVER_CONFIG }}/ dfs -copyFromLocal '{{ WORKING_DIR }}/resources/data/parquet/undefined_precision_numeric.parquet' '{{ HCFS_SCHEME }}{{ HCFS_BUCKET }}{{ TEST_LOCATION }}/parquet/undefined_precision_numeric.parquet'
-- end_ignore
...
{{ CLEAN_UP }}\!{{ HCFS_CMD }} --config {{ PXF_CONF }}/servers/{{ SERVER_CONFIG }}/ dfs -rm -r -f {{ HCFS_SCHEME }}{{ HCFS_BUCKET }}{{ TEST_LOCATION }}/parquet/undefined_precision_numeric.parquet
