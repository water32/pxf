...
\!{{ HCFS_CMD }} --config {{ PXF_BASE }}/servers/{{ SERVER_CONFIG }}/ dfs -copyFromLocal '{{ WORKING_DIR }}/resources/data/parquet/parquet_primitive_types' '{{ HCFS_SCHEME }}{{ HCFS_BUCKET }}{{ TEST_LOCATION }}/parquet/parquet_primitive_types'
-- end_ignore
...
{{ CLEAN_UP }}\!{{ HCFS_CMD }} --config {{ PXF_BASE }}/servers/{{ SERVER_CONFIG }}/ dfs -rm -r -f {{ HCFS_SCHEME }}{{ HCFS_BUCKET }}{{ TEST_LOCATION }}/parquet/parquet_primitive_types
