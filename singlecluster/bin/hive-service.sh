...
start() {
    ...
    nohup nice -n ${HIVE_NICENESS} "${HIVE_BIN}/hive" --config "${HIVE_ROOT}/conf" --service "${service}" "${ACTUAL_HIVE_OPTS}" &>"${log}" </dev/null &
    echo $! >"${pid}"
}
...
stop() {
    if [ -f "${pid}" ]; then
        ...
        if ! kill -0 "${TARGET_PID}" &>/dev/null; then
            TARGET_PID_REAL=$(ps -e -o pid=,ppid= | awk "/$TARGET_PID\$/{print \$1}")
        ...
        if kill -0 "${TARGET_PID}" &>/dev/null; then
            ...
            if kill -0 "${TARGET_PID}" &>/dev/null; then
                echo "$service did not stop gracefully after $HIVE_STOP_TIMEOUT seconds: sending signale SIGKILL"
                kill -9 "${TARGET_PID}"
            fi
        else
