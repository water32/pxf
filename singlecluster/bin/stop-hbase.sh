...
"${bin}/hbase" "${command}" stop &>/dev/null &

if kill -0 "${hbase_pid}" &>/dev/null; then
    echo "stopping ${command}"
    kill "${hbase_pid}"
    sleep "${hbase_stop_timeout}"
    if kill -0 "${hbase_pid}" &>/dev/null; then
        echo "$command did not stop gracefully after $hbase_stop_timeout seconds: sending signal SIGKILL to ${hbase_pid}"
        kill -9 "${hbase_pid}"
    fi
else
